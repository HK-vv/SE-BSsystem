...
**4）个人信息修改功能：**
管理员可以修改个人信息和账号密码，修改密码时需要验证当前密码满足复杂度约束条件，超级管理员可以帮助管理员进行密码重置。
...
<div>			<!--块级封装-->
    ...
    <br>		<!--换行-->
    图 4.3.1.1 自主组卷练习流程图	<!--标题-->
    <br>
    <img src="img\4.3.1.2.png" alt="4.3.1.2" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 4.3.1.2 自主组卷答题子功能流程图	<!--标题-->
    </center>
</div>
...
本模块为用户自主组卷模块，用来自主组卷练习，并转入答题功能，主要包含以下几个业务：
...
<div>			<!--块级封装-->
    ...
    <br>
    <img src="img\4.4.1.3.png" alt="4.4.1.3" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 4.4.1.2 用户答题流程图	<!--标题-->
    </center>
</div>
...
正常业务如图4.4.1.1 ， 4.1.1.2 ， 4.1.1.3所示
...
管理端界面为知识竞赛组织者对竞赛题目管理的网页端界面，整体采用vue3来进行编写。网页前端共包含个界面：1.登录界面；2.个人信息界面；3.管理员管理界面
...
此处界面主要为管理员和超级管理员修改个人信息而提供的。具体实现方式如下：

界面由用户名、邮箱、联系电话和新密码及确认密码的输入框和确认按钮组成
...
1.当点击用户信息管理这个按钮的时候，默认显示管理员信息列表，中上栏有用户/管理员信息切换按钮。当处于管理员信息界面时，若用户权限为管理员则可以查看所有管理员的信息，若该权限为超级管理员则可以创建、查看并删除任意管理员账号，且可以重置管理员密码。当处于用户信息界面时，显示所有小程序用户的信息（用户名、rating、参赛次数、比赛表现），点击比赛表现列“详情”按钮后会出现历次比赛列表及相应排名弹窗界面，点击比赛列表中某一比赛记录右侧答题情况按钮，即可查看该用户在对应比赛中的答题情况。

2.当点击比赛管理这个按钮的时候，网页的中上栏显示创建比赛按钮，及搜索框，下栏则显示比赛列表且每行有对应的分析、公布、编辑、删除按钮。

3.当点击题库管理的时候，上栏显示由条件搜索题目的搜索框、添加题目按钮、批量删除按钮和批量公开按钮；下栏则显示题库中的题目。在添加题目这个弹窗界面中支持以Excel文件批量导入功能
