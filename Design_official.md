...
| 5    | 按钮   | 保存         | 按钮提示 | 点击后可保存修改后的个人信息，显示在弹窗中   |

**创建管理员账号**
| 序号 | 类型 | 信息内容 | 信息表现 | 说明 |
| ---- | ---- | --------| ---------| -----|
| 1 | 输入框 | 用户名 | 字符 | 新建管理员用户名 |
| 2 | 文本 | 确认信息 | 字符提示| 提示是否确认创建 |
| 3 | 按钮 | 创建 | 按钮提示 | 选择是否确认创建 |
...
| 查找比赛功能               | D3         | （超级）管理员在比赛管理界面查找比赛                               |
| 查看比赛信息功能             | D4         | （超级）管理员在比赛管理界面查看比赛信息                               |
| 修改比赛信息功能             | D5         | （超级）管理员在比赛管理界面修改比赛信息                               |
| 删除比赛功能              | D6         | （超级）管理员在比赛管理界面删除比赛                               |
| 查看比赛排名与结果分析功能 | D7         | （超级）管理员可以在比赛结束后查看比赛排行榜和结果分析             |
| 公布结果功能               | D8        | （超级）管理员需在比赛结束后公布比赛结果，以便用户查看个人答题情况 |
...
（超级）管理员可以创建比赛，点击创建比赛按钮后出现创建比赛弹窗。（超级）管理员需确定比赛名称，选择比赛开始时间、最晚开始时间、答题时间、是否为私密比赛、是否 rating 属性，选定赛题以创建比赛。
...
| 3    | 选项框 | 比赛最晚开始时间 | 时间     | 选择比赛最晚开始时间，在比赛开始时间与比赛最晚开始时间之间开始答题均可，必选 |
...
| 10   | 单选框 | 是否 rating      | 选项     | 告知用户该次比赛是否计入 rating，实际操作以赛后设置为准，必选                |
...
（超级）管理员可以在比赛管理功能处看到包含所有比赛信息的列表，按时间顺序排列。点击每行比赛记录后的“排行榜”按钮可以查看对应比赛的排名情况，其中包含用户的正确里率和比赛用时（不一定），点击“结果分析”可以查看比赛结果分析，包括用户答题正确率分布直方图，错题分布。
...
<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D4_rating.png" alt="D4_rating" style="zoom:60%;"/>
    <br>		<!--换行-->
