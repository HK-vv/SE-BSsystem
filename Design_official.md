...
配合开源开发框架Django, 采用了经典的三层架构, 即表示层, 逻辑层, 和数据访问层.

表示层又分为用户端和管理端, 和逻辑层用团队定义的接口交互. 逻辑层和数据访问层的交互主要使用Django提供的ORM接口.
...
（超级）管理员可在个人中心对自己的个人信息进行修改，同时超级管理员可以创建、删除管理员账号并且重置管理员的密码
...
| 3    | 选项框 | 比赛最晚开始时间 | 时间     | 选择比赛最晚开始时间。用户在比赛开始时间与比赛最晚开始时间之间开始答题均可，必选 |
...
（超级）管理员可以查看和修改（尚未开始）比赛信息，包括创捷比赛时的所有设置，并可根据需求设置每页显示的数量。
...
在用户比赛界面展示所有比赛简述条目，条目内容包括用户的参赛情况，比赛的状态（进行中，未开始，已结束）。并提供相应的D1比赛答题（进行中的比赛）或D9比赛回顾（已结束且以参赛的比赛）入口。
...
#### 6.9.1 功能简述
用户可以比赛回顾页面查看本次比赛信息：题目信息、本次得分、本次耗时、本次排名、本次分数；排行榜信息：本次前三名与用户ID、本次前三名与用户得分、本次前三名与用户耗时、本次前三名与用户总rating、本次前三名与用户总rating_change。用户可以点击本次题目按钮进入答题页面。
