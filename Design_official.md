...
| （超级）管理员个人信息修改功能 | A6         | （超级）管理员可在个人中心对个人信息进行修改，并且支持添加、修改以及重置管理员账号（仅限超级管理员）                                                                             |
...
| 5 | 文本 | 联系方式输入框 | 字符提示| 遵守字符限制，显示在联系方式输入框里 |
| 6    | 输入框 | 联系方式输入框   | 字符提示 | 输入对应修改联系方式，显示在修改框中       |
...
| 序号 | 类型 | 信息内容 | 信息表现 | 说明 |
| ---- | ---- | --------| ---------| -----|
| 1 | 搜索输入框 | 用户名 | 字符 | 根据用户名搜索管理员信息 |
| 2 | 下拉选项框 | 每页用户数量选项框 | 选项 |
| 3 | 文本 | 确认信息 | 字符提示| 提示是否确认重置 |
| 4 | 按钮 | 选中 | 按钮提示 | 选择是否选中对应管理员 |
| 5 | 按钮 | 重置 | 按钮提示 | 选择是否确认重置 |
...
该功能具有基本的题目添加功能，超级管理员和管理员均拥有该功能权限。添加题目时需在题库管理界面，点击添加题目按钮，在弹出对话框中选择单个添加（默认）/批量导入。<br/>
...
该功能具有基本的题目（批量）公开功能，超级管理员有对题库中所有题目的公开权限，管理员仅有公开自己创建的题目的权限。可以直接在题库管理界面勾选需要公开的题目，并点击“公开”按钮，在确认对话框中再次确认后，即可对相应题目进行公开。**(因对于公开状态的题目，用户可通过随机组卷的方式在竞赛前获得，为保证公平性，“是否公开”属性仅支持单向的从不公开改为公开状态。）**对于选中的原本为公开状态的题目会向用户反馈这些题目的id进行提示，不会进行状态修改。
...
| 查看比赛信息功能     | D3         | （超级）管理员在比赛管理界面查看比赛信息                           |
| 修改比赛信息功能     | D4         | （超级）管理员在比赛管理界面修改比赛信息                           |
...
（超级）管理员可以在比赛管理功能处看到包含所有比赛信息的列表，按时间顺序排列。点击“查看详情”可以查看比赛的设置信息。可以根据需求选择每页显示的比赛数量、根据比赛名称检索比赛信息。

#### 6.3.2 功能实现流程

<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D3_info.png" alt="D3_info" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 6.3.2 查看比赛信息功能流程图	<!--标题-->
    </center>
</div>
...
| 2    | 文本       | 参赛人数       | 字符     | 参加比赛的人数，显示在比赛名称之后                   |
| 3    | 按钮       | 查看详情       | 字符     | 点击查看详细比赛设置，显示在比赛名称之后             |
| 4    | 搜索输入框 | 比赛名称关键字 | 字符     | 根据比赛名称搜索比赛信息，显示在比赛信息栏顶部       |
...
（超级）管理员可以修改（尚未开始）比赛信息，包括创捷比赛时的所有设置。
...
| 序号 | 类型 | 信息内容     | 信息表现 | 说明                                 |
| ---- | ---- | ------------ | -------- | ------------------------------------ |
| 1    | 按钮 | 修改比赛信息 | 字符     | 点击修改比赛信息，显示在比赛名称之后 |
| 2    | 弹窗 | 比赛信息     | 弹窗     | 与创建比赛时的界面一致               |
...
<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D5_delete.png" alt="D5_delete" style="zoom:60%;"/>
    <br>		<!--换行-->
...
| 序号 | 类型 | 信息内容 | 信息表现 | 说明                               |
| ---- | ---- | -------- | -------- | ---------------------------------- |
| 1    | 按钮 | 删除比赛 | 字符     | 点击将比赛删除，显示在比赛名称之后 |
...
在比赛信息列表点击每行比赛记录后的“排行榜”按钮可以查看对应比赛的排名情况，其中包含用户的正确里率和比赛用时（不一定），点击“结果分析”可以查看比赛结果分析，包括用户答题正确率分布直方图，错题分布。

#### 6.6.2 功能实现流程

<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D6_after_game.png" alt="D6_all_game" style="zoom:60%;"/>
    <br>		<!--换行-->
...
| 序号 | 类型 | 信息内容 | 信息表现 | 说明                       |
| ---- | ---- | -------- | -------- | -------------------------- |
| 1    | 按钮 | 排行榜   | 按钮     | 点击后弹窗显示比赛排行榜   |
| 2    | 按钮 | 结果分析 | 按钮     | 点击后弹窗显示比赛结果分析 |
