...
#### 3.7.2 功能实现流程// COMMENT: 同上, 此外图片加一下标题
...
该功能具有基本的题目筛选和查看功能，超级管理员和管理员均拥有该功能权限。可以直接在题库管理界面对当前题库中所有题目信息进行查看。也可根据题目 id、题目类型、题目描述、题目标签、是否公开中任一或多个属性信息对当前题库中的题目进行筛选。
...
#### 4.4.5 异常处理 // COMMENT: 权限不足导致的删除失败?
...
#### 4.5.5 异常处理// COMMENT: 权限不足导致公开失败?

| 序号 | 错误编码 | 错误信息   | 说明/处理办法                                                                |
| ---- | -------- | ---------- | ---------------------------------------------------------------------------- |
| 1    | B5-E01   | 登录失效   | 操作后检测到当前 cookie 已过期，提示登录失效并跳转到登录界面                 |
| 1    | B5-E02   | 题目已公开 | 对于已公开的题目，如再次勾选并点击“公开”，将会给出提示，不会改变题目公开状态 |
...
#### 5.1.2 功能实现流程 // COMMENT: 第三个框包含填空操作, 显示答案是否正确之外还会显示正确答案, 图片加标题
...
| 修改比赛功能         | D4         | （超级）管理员在比赛管理界面修改比赛信息                     |
| 删除比赛功能         | D5         | （超级）管理员在比赛管理界面删除比赛                         |
| 查看赛后信息         | D6         | （超级）管理员可以在比赛结束后查看比赛排行榜和结果分析       |
| 公布结果功能         | D7         | （超级）管理员需在比赛结束后公布比赛结果，以便用户查看个人答题情况 |
| 用户查看比赛简述功能 | D8         | 用户可以在比赛查看页面查看所有比赛及其状态                   |
| 用户比赛回顾功能     | D9         | 用户可以在比赛回顾页面查看比赛信息和排行榜信息                |
...
| 8    | 单选框 | 是否为私密比赛   | 选项     | 必选                                                         |
| 9    | 输入框 | 比赛密码         | 字符     | 若为私密比赛需设置参赛密码，用户输入正确的参赛密码即可参加比赛 |
...
| 16   | 选项框 | 题目是否为公开   | 选项     | 根据题目是否为公开筛选题目，显示在选择考题界面               |
...
（超级）管理员可以查看和修改（尚未开始）比赛信息，包括创建比赛时的所有设置，并可根据需求设置每页显示的数量。

#### 6.4.2 功能实现流程

<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D4_modify.png" alt="D4_modify" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 6.4.2 修改比赛信息功能流程图	<!--标题-->
    </center>
</div>
...
| 2    | 按钮       | 比赛设置     | 字符     | 点击查看详细比赛设置，显示在比赛名称之后             |
| 3    | 弹窗       | 比赛信息     | 弹窗     | 与创建比赛时的界面一致                               |
| 4    | 下拉选项框 | 每页比赛数量 | 选项     | 选择每页比赛数量，下拉选项框形式显示在比赛信息栏底部 |
...
| 3    | D4-E03   | 登录失效       | 操作后检测到当前 cookie 已过期，提示登录失效并跳转到登录界面                                   |

### 6.5 功能 D5 实现// COMMENT: 将删除功能移至D4

#### 6.5.1 功能简述

（超级）管理员可以在比赛管理部分批量删除比赛。

#### 6.5.2 功能实现流程

<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D5_delete1.png" alt="D5_delete1" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 6.5.2 删除比赛功能流程图	<!--标题-->
    </center>
</div>
...
| 序号 | 类型   | 信息内容 | 信息表现 | 说明                                     |
| ---- | ------ | -------- | -------- | ---------------------------------------- |
| 1    | 按钮   | 删除比赛 | 字符     | 点击将比赛删除，显示在比赛信息栏顶部     |
| 2    | 勾选框 |          |          | 点击选择要删除的比赛，显示在比赛名称之前 |
...
| 序号 | 错误编码 | 错误信息 | 说明/处理办法                                                                      |
| ---- | -------- | -------- | ---------------------------------------------------------------------------------- |
| 1    | D5-E01   | 无权限   | 管理员仅能删除自己创建的比赛，超级管理员可以删除所有比赛，对超越权限的操作提示警告 |
| 2    | D5-E02   | 登录失效 | 操作后检测到当前 cookie 已过期，提示登录失效并跳转到登录界面                       |

### 6.6 功能 D6 实现

#### 6.6.1 功能简述

在比赛信息列表点击每行比赛记录后的赛后分析按钮可以查看对应比赛的排名情况和用户答题情况，包括用户答题正确率分布直方图，错题分布。

#### 6.6.2 功能实现流程

<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D6_after_game1.png" alt="D6_after_game1" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 6.6.2 查看赛后信息功能流程图	<!--标题-->
    </center>
</div>
...
| 序号 | 类型 | 信息内容 | 信息表现 | 说明                                   |
| ---- | ---- | -------- | -------- | -------------------------------------- |
| 1    | 按钮 | 赛后分析 | 按钮     | 点击后弹窗显示比赛排行榜和用户答题情况 |
...
| 序号 | 错误编码 | 错误信息 | 说明/处理办法                                                |
| ---- | -------- | -------- | ------------------------------------------------------------ |
| 1    | D6-E01   | 登录失效 | 操作后检测到当前 cookie 已过期，提示登录失效并跳转到登录界面 |
...
<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D7_rating.png" alt="D7_rating" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 6.7.2 更新 rating 功能流程图	<!--标题-->
    </center>
</div>
...
| 序号 | 错误编码 | 错误信息 | 说明/处理办法                                                                                |
| ---- | -------- | -------- | -------------------------------------------------------------------------------------------- |
| 1    | D7-E01   | 无权限   | 管理员仅能公布自己创建的比赛的结果，超级管理员可以公布所有比赛结果，对超越权限的操作提示警告 |
| 2    | D7-E02   | 登录失效 | 操作后检测到当前 cookie 已过期，提示登录失效并跳转到登录界面                                 |

### 6.8 功能D8实现

#### 6.8.1 功能简述

在用户比赛界面展示所有比赛简述条目，条目内容包括用户的参赛情况，比赛的状态（进行中，未开始，已结束）。并提供相应的 **D1** 比赛答题（进行中的比赛）或 **D9** 比赛回顾（已结束且以参赛的比赛）入口。

#### 6.8.2 功能实现流程

<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\D8.png" alt="D8" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 6.8 用户查看比赛概述功能流程图	<!--标题-->
    </center>
</div>
...
| 序号 | 错误编码 | 错误信息 | 说明/处理办法                                                |
| ---- | -------- | -------- | ------------------------------------------------------------ |
| 1    | D8-E01   | 登录失效 | 操作后检测到当前 cookie 已过期，提示登录失效并跳转到登录界面 |

### 6.9 功能D9实现

#### 6.9.1 功能简述
用户可以比赛回顾页面查看本次比赛信息：题目信息、本次得分、本次耗时、本次排名、本次分数；排行榜信息：本次前三名与用户的ID、得分、耗时、赛前rating和rating改变量.。用户可以点击题目按钮进入展示页面。


#### 6.9.2 功能实现流程// COMMENT: 图片标题写好
<div>			<!--块级封装-->
    <center>	<!--将图片和文字居中-->
    <img src="img\real_Design_official_6.9.2.png" style="zoom:60%;"/>
    <br>		<!--换行-->
    图 6.9 	<!--标题-->
    </center>
</div>
...
| 2    | 文字 | 排行榜前三名与用户信息：排名、ID、得分、耗时、总rating、总rating_change      | 文本框     |  |
